      ...
      {changelogs.map((changelog, index) => (
        ...
        >
          ...
          <div className="flex flex-col pb-10 sm:pb-12 flex-grow">
            <div className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 group-hover:text-pink-400 transition-colors duration-300">
              {changelog.title}
            </div>
            ...
            >
              <div className="flex flex-wrap gap-2 mb-3 sm:mb-4">
                {changelog.tag.map((tag) => (
                  <span
                    key={tag}
                    className="bg-pink-50 text-pink-400 px-3 py-1 rounded-full text-sm font-medium"
                  >
                    {tag}
                  </span>
                ))}
              </div>
              <h2 className="title-font text-base sm:text-lg md:text-xl text-gray-500 mb-2">
                {changelog.date}
              </h2>
                ...
                <ReactMarkdown
                  components={{
                    ...
                    img: ({ node, ...props }) => (
                      ...
                      <img
                        {...props}
                        className="mt-6 sm:mt-8 mb-4 rounded-xl shadow-sm"
                        alt={props.alt || ""}
                      />
                    ),
                  }}
